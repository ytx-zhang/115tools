...
- 配置完整（`refresh_token`、三个路径、`strm_url` 齐全）时同步器自动拉起；缺失项会在面板顶部高亮提示，补齐保存后无需重启即自动开始同步
- 首次启动会扫描云端目录建立本地数据库索引；`sync_path` 的云端 FID 每次启动都会与云端重新核对，云端目录被手动重建后也能自动识别并重新扫描；`temp_path` 的 FID 仅存于内存、不落库，避免云端重建残留旧索引
