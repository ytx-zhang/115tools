...
func (c *Client) AddOfflineTasks(ctx context.Context, urls []string, saveDirID, savePath string) ([]OfflineAddResult, error) {
	...
	if err == nil {
		...
		log(ctx, "添加离线下载任务", nil, dur, "数量", len(urls), "路径", savePath, "成功", success)
	} else {
		log(ctx, "添加离线下载任务", err, dur, "数量", len(urls), "路径", savePath)
	}
	return res, err
}
...
func (c *Client) ListTasks(ctx context.Context, page int) (*OfflineTaskPage, error) {
	...
	if err != nil {
		log(ctx, "获取云端任务列表", err, dur, "页码", max(page, 1))
		return nil, err
	}
	log(ctx, "获取云端任务列表", nil, dur, "页码", max(page, 1), "任务总数", res.Count, "本页条数", len(res.Tasks))
	return &res, nil
}
...
func (c *Client) DeleteTask(ctx context.Context, infoHash string, deleteFiles bool) error {
	...
	log(ctx, "删除离线任务", err, dur, "info_hash", infoHash)
	return err
}
...
func (c *Client) ClearTasks(ctx context.Context, flag int) error {
	...
	log(ctx, "批量清除任务", err, dur, "flag", flag)
	return err
}
...
func (c *Client) GetQuota(ctx context.Context) (*OfflineQuota, error) {
	...
	if err != nil {
		log(ctx, "获取云下载配额", err, dur)
		return nil, err
	}
	log(ctx, "获取云下载配额", nil, dur, "总数", res.Count, "已用", res.Used, "剩余", res.Surplus)
	return &res, nil
}
...
func AddTorrentFromData(ctx context.Context, c *Client, torrentData []byte, torrentName, savePath string) (*OfflineAddResult, error) {
	t0 := time.Now()
	journal.Debug(ctx, "添加种子离线任务", "文件名", torrentName, "save_path", savePath)
	...
	journal.Info(ctx, "添加种子离线任务完成", "文件名", torrentName, "info_hash", r.InfoHash, "save_path", savePath, "耗时", time.Since(t0))
	return &r, nil
}
