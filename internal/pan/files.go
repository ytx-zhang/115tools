...
func (c *Client) GetFileList(ctx context.Context, cid string) ([]FileInfo, error) {
	...
	for {
		...
		log(ctx, "拉取云端文件列表", err, dur, "页码", offset/1150+1, "条数", len(files))
		if err != nil {
			return nil, err
		}
		if len(files) == 0 {
...
func (c *Client) GetDownloadURL(ctx context.Context, pickCode, ua string) (*DownloadURLInfo, error) {
	...
	if err != nil {
		log(ctx, "获取下载直链", err, dur, "pickcode", pickCode)
		return nil, err
	}
	if items.Value == nil || len(*items.Value) == 0 {
		log(ctx, "获取下载直链", fmt.Errorf("未提取到下载信息"), dur, "pickcode", pickCode)
		return nil, fmt.Errorf("未提取到下载信息")
	}
	for _, item := range *items.Value {
		if item.URL.URL == "" {
			log(ctx, "获取下载直链", fmt.Errorf("115接口返回空直链"), dur, "pickcode", pickCode)
			return nil, fmt.Errorf("115接口返回空直链")
		}
		log(ctx, "获取下载直链", nil, dur, "pickcode", pickCode, "文件名", item.FileName)
...
func (c *Client) CreateFolder(ctx context.Context, pid, name, path string) (string, error) {
	...
	if err == nil {
		log(ctx, "云端创建目录", nil, dur, "路径", path)
	...
	log(ctx, "云端创建目录", err, dur, "路径", path)
	return "", err
}
...
func (c *Client) MoveFile(ctx context.Context, fid, cid, path string) error {
	...
	log(ctx, "云端移动文件", err, dur, "路径", path, "数量", fidCount(fid))
	return err
}
...
func (c *Client) DeleteFile(ctx context.Context, fid, path string) error {
	...
	log(ctx, "云端删除文件", err, dur, "路径", path, "数量", fidCount(fid))
	return err
}
...
func (c *Client) RenameFile(ctx context.Context, fid, name, path string) (string, error) {
	...
	if err == nil {
		log(ctx, "云端重命名", nil, dur, "路径", path, "新文件名", res.FileName)
	} else {
		log(ctx, "云端重命名", err, dur, "路径", path)
	}
...
func (c *Client) GetDirInfo(ctx context.Context, path string) (*DirInfo, error) {
	...
	if err != nil {
		log(ctx, "查询云端目录信息", err, dur, "路径", path)
		return nil, err
	}
	if info.Value == nil {
		log(ctx, "查询云端目录信息", fmt.Errorf("未获取到目录信息"), dur, "路径", path)
		return nil, fmt.Errorf("未获取到目录信息")
	}
	log(ctx, "查询云端目录信息", nil, dur, "路径", path, "文件数", info.Value.FileCount, "目录数", info.Value.FolderCount)
